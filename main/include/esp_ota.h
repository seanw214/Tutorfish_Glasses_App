...
typedef void *esp_littlefs_ota_handle_t;
 ...
 */
esp_err_t esp_https_ota(const esp_http_client_config_t *config, char *local_response_buffer);
 ...
 *    - For other return codes, refer OTA documentation in esp-idf's app_update component.
 */
esp_err_t esp_https_ota_perform(esp_https_ota_handle_t https_ota_handle, FILE *f);
 ...
 *
 * @param[in]   https_ota_handle pointer to esp_https_ota_handle_t structure
 * @param[in]   FILE pointer to the littlefs instance that the downloaded .bin will be saved into
 *
 * @return
 *    - false
 *    - true
...
esp_err_t esp_https_app_ota(const esp_http_client_config_t *config, char *app_name);
